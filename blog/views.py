...
def index(request):
    most_popular_posts = Post.objects.annotate(count_likes=Count('likes')).order_by('-count_likes').prefetch_related('author')[:5]
    ...
    most_popular_tags = Tag.objects.annotate(count_posts=Count('posts')).order_by('-count_posts')[:5]
